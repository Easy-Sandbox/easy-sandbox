...
> .env file > defaults.
~/.sbox/config.toml is an optional SDK-extension source loaded between .env and defaults.
"""
...
_SBOX_DIR = Path.home() / ".sbox"  # SDK extension
_CONFIG_FILE = _SBOX_DIR / "config.toml"  # SDK extension
_ENV_FILE_CANDIDATES = [Path(".env"), _SBOX_DIR / ".env"]
...
def load_config(**overrides: Any) -> TransportConfig:
    ...
    3. .env file values
    4. ~/.sbox/config.toml (optional SDK extension)
