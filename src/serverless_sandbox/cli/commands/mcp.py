...
def install(ctx: click.Context, target: str) -> None:
    ...
    if target == "vscode":
        ...
        mcp_servers["serverless-sandbox"] = server_config
...
def start(
    ...
    """
    from serverless_sandbox.agent.mcp import SandboxMCPServer
...
def status(ctx: click.Context) -> None:
    ...
    from serverless_sandbox.agent.tools import TOOL_SCHEMAS
    ...
    for ide_name, path_fn in _IDE_CONFIG_MAP.items():
        try:
            ...
            if config_path.is_file():
                ...
                installed = "serverless-sandbox" in config.get(key, {})
