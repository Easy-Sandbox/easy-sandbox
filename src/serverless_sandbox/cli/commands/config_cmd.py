...
_SBOX_DIR = Path.home() / ".sbox"
_CONFIG_FILE = _SBOX_DIR / "config.toml"
...
def _save_config_dict(data: dict[str, Any]) -> None:
    ...
    _SBOX_DIR.mkdir(parents=True, exist_ok=True)
    lines = ["[transport]"]
...
def _write_env_api_key(api_key: str) -> None:
    """Write or update E2B_API_KEY in ~/.sbox/.env file."""
    _SBOX_DIR.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    found = False
...
def _read_env_api_key() -> str | None:
    """Read E2B_API_KEY from ~/.sbox/.env file."""
