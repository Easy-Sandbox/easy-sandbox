...
def _find_local_template(template_id: str) -> SandboxTemplate | None:
    """Scan ``~/.ebx/templates/`` for a ``template.yaml`` that
    ...
    :class:`~easy_sandbox.models.errors.TemplateParseError` is raised
...
async def resolve_capabilities(
    ...
    *,
    ...
    2. Scan ``~/.ebx/templates/`` for a matching template name.
