...
_INSTALL_TARGETS = {
    "project": ".ebx/skills",
    "global": str(Path.home() / ".ebx" / "skills"),
...
def install(ctx: click.Context, skill_ref: str, target: str) -> None:
    ...
    示例：\n
      ebx skill install code-analysis\n
      ebx skill install owner/repo --target cursor
    """
    fmt = get_formatter(ctx)
...
def list_skills(ctx: click.Context, target: str) -> None:
    ...
    if target in ("project", "all"):
        targets_to_check.append(("project", Path(".ebx/skills")))
    if target in ("global", "all"):
        targets_to_check.append(("global", Path.home() / ".ebx" / "skills"))
...
def publish(ctx: click.Context, path: str) -> None:
    ...
    fmt.print_success(
        ...
        "  3. Others can install via: ebx skill install owner/repo"
    )
...
## 概述
{skill_ref} Skill — 通过 ebx skill install 安装。
