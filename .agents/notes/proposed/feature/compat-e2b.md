...
## Problem
E2B users need a migration path to switch from the official E2B SDK to Serverless Sandbox with minimal code changes.

## Decision
`compat/sandbox.py` provides E2B-compatible `Sandbox` wrapper:
- Same method names and signatures as `e2b` Python SDK
- `from serverless_sandbox.compat import Sandbox` as drop-in replacement
- Translates E2B-style calls to native Serverless Sandbox API calls
...
## Acceptance criteria
- `from serverless_sandbox.compat import Sandbox` is a drop-in replacement for `from e2b import Sandbox`
