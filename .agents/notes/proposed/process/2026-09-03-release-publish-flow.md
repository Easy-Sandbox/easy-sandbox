...
1. **Local install / E2E test passes** — validate the template locally (`sbox install ./path --registry-type local`, create sandbox, run its custom commands).
...
3. **Real GitHub install E2E passes** — `sbox install owner/repo --registry-type github` end-to-end against the published release must succeed.
...
## Dependencies
- `sbox install` (github + local registry types) — see `docs/design/cli-design.md`
...
## Test Strategy
- Local: `sbox install ./template --registry-type local` → create → `sbox run` custom command succeeds.
- Published: `sbox install owner/repo --registry-type github` resolves the release and installs.
...
- A real `sbox install owner/repo --registry-type github` E2E is green after the user publishes.
